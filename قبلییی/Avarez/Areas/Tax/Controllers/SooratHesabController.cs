...
namespace Avarez.Areas.Tax.Controllers
{
    public class SooratHesabController : Controller
    {
        ...
        // خواندن private key با BouncyCastle
        public static RSA LoadPrivateKey(string pem)
        {
            ...
            throw new Exception("Invalid PEM private key.");
        }
        ...
        public ActionResult SamaneMoadian(long HeaderId)
            ...
            string path = "";
                ...
                // مسیر Private Key
                string privateKeyPath = @"C:\Keys\privateKey.pem";
                string privateKeyPem = System.IO.File.ReadAllText(privateKeyPath);

                InvoiceDto item = CreateValidInvoice(uid, HeaderId);
                List<InvoiceDto> list1 = new List<InvoiceDto>();
                list1.Add(item);
                List<InvoiceDto> invoiceList = list1;

                string jsonInvoice = JsonConvert.SerializeObject(invoiceList);

                // امضای دیجیتال صورتحساب
                string signature = SignData(jsonInvoice, privateKeyPem);

                // آماده‌سازی پکت ارسال
                var packet = new
                {
                    uid = uid,
                    packetType = "INVOICE",
                ...
                // ارسال به سرویس دارایی
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri("https://tp.tax.gov.ir/requestsmanager/");
                    var content = new StringContent(jsonPacket, Encoding.UTF8, "application/json");

                    HttpResponseMessage response = client.PostAsync("api/v2/invoice", content).Result;

                    Console.WriteLine("Status Code: " + response.StatusCode);
                    Console.WriteLine("Headers: " + response.Headers.ToString());

                    string result = response.Content.ReadAsStringAsync().Result;
                    Console.WriteLine("Response: " + (string.IsNullOrEmpty(result) ? "<خالی>" : result));
                }
                /*
