//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Avarez.Models
{
    using System;
    
    public partial class sp_Municipality_LogSelect
    {
        public int fldID { get; set; }
        public string نام { get; set; }
        public int شهر { get; set; }
        public string کد_انفرماتیک { get; set; }
        public byte کد_خدمت { get; set; }
        public string نام_کاربری_از_وب_سرویس_عوارض { get; set; }
        public string رمز_عبور_از_وب_سرویس_عوارض { get; set; }
        public string توضیحات { get; set; }
        public string تاریخ_ایجاد { get; set; }
        public string کاربر_ایجاد_کننده { get; set; }
        public string نوع_تغییرات { get; set; }
        public Nullable<System.TimeSpan> ساعت_تغییرات { get; set; }
        public string موقعیت_کاربر_ایجاد_کننده { get; set; }
    }
}
