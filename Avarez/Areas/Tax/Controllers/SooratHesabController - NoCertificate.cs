...
namespace Avarez.Areas.Tax.Controllers
{
    public class SooratHesabController : Controller
    {
        ...
        {//باز شدن تب جدید
            ...
            var header = new TaxCollectData.Library.Dto.Content.InvoiceHeaderDto();
            header.Inty = 1;
            header.Inp = 1;
            header.Inno = "1234567890";
            header.Tins = "5555555555";
        ...
        private static InvoiceDto CreateValidInvoice(string MemoryId, long HeaderId)
        {
            ...
            List<InvoiceBodyDto> bodylist = new List<InvoiceBodyDto>();
            foreach (var item in list)
            {
                TaxCollectData.Library.Dto.Content.InvoiceBodyDto bd = new TaxCollectData.Library.Dto.Content.InvoiceBodyDto
                {
                    Sstid = item.fldsstid,
                    Sstt = item.fldsstt,
                    Mu = item.fldmu,
...
*/
        ...
        public async System.Threading.Tasks.Task<ActionResult> SamaneMoadian(long HeaderId)
        {
            ...
            try
            {
                ...
                string BASE_URL = "https://tp.tax.gov.ir/req/api/";
                string CLIENT_ID =  select2.fldUniqId;
                string PRIVATE_KEY_PEM_PATH = @"C:\Keys\privateKey.pem";
                if (CLIENT_ID == "A3HHG7")
                    PRIVATE_KEY_PEM_PATH = @"C:\Keys\PrivateM.pem";
                ...
                Console.WriteLine("=== شروع ارسال فاکتور با SDK مودیان ===");
                ...
                // مرحله 4: ایجاد و ارسال فاکتور
                var result = await SendSampleInvoice(CLIENT_ID, HeaderId, Convert.ToInt64(Session["TaxUserId"]));
                Thread.Sleep(10_000);
        ...
        // تست ساده قبل از استفاده از SDK
        public static async System.Threading.Tasks.Task TestConnection()
        {
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    var response = await client.GetAsync("https://tp.tax.gov.ir/requestsmanager/api/v2/nonce");
        ...
        /// پیکربندی اولیه SDK
        /// </summary>
        private static async System.Threading.Tasks.Task InitializeTaxApiService(string CLIENT_ID, string PRIVATE_KEY_PEM_PATH, string BASE_URL)
        {
            Console.WriteLine("پیکربندی SDK...");
            System.Net.ServicePointManager.SecurityProtocol =
    System.Net.SecurityProtocolType.Tls12 |
    ...
    System.Net.SecurityProtocolType.Tls;
            try
            {
                // استفاده از Pkcs8SignatoryConfig برای کلید خصوصی PEM
                TaxApiService.Instance.Init(
                    CLIENT_ID,
                    new Pkcs8SignatoryConfig(PRIVATE_KEY_PEM_PATH, null),
                    new NormalProperties(ClientType.SELF_TSP), // خود مودی (نه شرکت معتمد)
                    BASE_URL
                );

                Console.WriteLine("✅ SDK با موفقیت پیکربندی شد");
            }
            catch (Exception ex)
            {
                throw new Exception($"خطا در پیکربندی SDK: {ex.Message}", ex);
            }
        }

        /// <summary>
        /// دریافت اطلاعات سرور (کلید عمومی سازمان)
        /// </summary>
        private static async System.Threading.Tasks.Task GetServerInformation()
        {
            Console.WriteLine("دریافت اطلاعات سرور...");

            try
            {
                var serverInfo = await TaxApiService.Instance.TaxApis.GetServerInformationAsync();

                if (serverInfo != null)
                {
                    Console.WriteLine($"✅ اطلاعات سرور دریافت شد");
                }
            }
            catch (HttpRequestException ex)
            {
                Console.WriteLine($"خطای HTTP: {ex.Message}");
                if (ex.InnerException != null)
                {
        ...
        /// دریافت توکن دسترسی
        /// </summary>
        private static async System.Threading.Tasks.Task RequestAccessToken()
        {
            Console.WriteLine("دریافت توکن دسترسی...");
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"✅ توکن دسترسی دریافت شد");
                    Console.WriteLine($"طول عمر توکن: {token.ExpiresIn} ثانیه");
                }
                else
                {
                    throw new Exception("توکن دسترسی دریافت نشد");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"خطا در دریافت توکن: {ex.Message}", ex);
            }
        }

        /// <summary>
        /// ایجاد و ارسال فاکتور نمونه
        /// </summary>
        private static async System.Threading.Tasks.Task<object> SendSampleInvoice(string CLIENT_ID, long HeaderId,long TaxUserId)
        {
            cartaxtest2Entities entities = new cartaxtest2Entities();
            prs_SelectHeaderSooratHesab hesab = entities.prs_SelectHeaderSooratHesab(new long?(HeaderId)).FirstOrDefault<prs_SelectHeaderSooratHesab>();
            List<prs_SelectDetailSooratHesab> list = entities.prs_SelectDetailSooratHesab(new long?(HeaderId)).ToList<prs_SelectDetailSooratHesab>();
            ...
            try
            {
                ...
                long randomSerialDecimal = random.Next(100000, 999999999);
                var now = DateTimeOffset.Now.ToUnixTimeMilliseconds();

                // تولید شماره مالیاتی
                string taxId = TaxApiService.Instance.TaxIdGenerator.GenerateTaxId(
                    CLIENT_ID,
                    randomSerialDecimal,
                    DateTime.Now
                );
                ...
                // ایجاد body فاکتور (اقلام)
                List<InvoiceBodyDto> bodyItem = new List<InvoiceBodyDto>();
                foreach (var item in list)
                {
                    var bd = new InvoiceBodyDto
                    {
                        Sstid = item.fldsstid,
                        Sstt = item.fldsstt,
                        Mu = item.fldmu,
                ...
                // ایجاد payment فاکتور (اختیاری برای پرداخت‌های کارتی)
                //var payment = new PaymentDto
                //{
                //    Iinn = "1111111111",     // شماره سوییچ
                //    Acn = "2222222222",      // شماره پذیرنده
                //    Trmn = "33333333",       // شماره پایانه
                //    Trn = "444444444",       // شماره پیگیری
                //    Pdt = now,               // تاریخ پرداخت
                //    Pmt = 1,                 // روش پرداخت: کارت
                //    Pv = 1090000             // مبلغ پرداختی
                //};
                ...
                var invoice = new InvoiceDto
                {
                    Header = header,
                    Body = bodyItem/* ,
                    Payments = new List<PaymentDto> { payment }*/
                };

                var invoices = new List<InvoiceDto> { invoice };

                // ارسال فاکتور
                var response = await TaxApiService.Instance.TaxApis.SendInvoicesAsync(invoices, null);

                if (response?.Body?.Result != null && response.Body.Result.Count > 0)
                {
                    var result = response.Body.Result;
                    var packetResponse = response.Body.Result.First();

                    string SerializeObjectErsal = Newtonsoft.Json.JsonConvert.SerializeObject(invoices);

                    entities.prs_tblSooratHesabStatusInsert(HeaderId, 4, "", packetResponse.ReferenceNumber, SerializeObjectErsal, packetResponse.Uid, TaxUserId, "1");
                    // برگرداندن نتیجه
                    return new
                    {
                        Success = true,
                        ...
                        Timestamp = DateTimeOffset.FromUnixTimeMilliseconds(response.Body.Timestamp),
                        Message = "فاکتور با موفقیت ارسال شد"
                    };
                }
                else
                {
                    throw new Exception("پاسخ ارسال فاکتور معتبر نیست");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"خطا در ارسال فاکتور: {ex.Message}", ex);
            }
        }

        /// <summary>
        /// استعلام وضعیت فاکتور ارسال شده
        /// </summary>
        private static async System.Threading.Tasks.Task InquireInvoiceStatus(string uid, string referenceNumber, string CLIENT_ID)
        {
        ...
        public static class ErrorHandler
        {
            ...
            {
                Console.WriteLine($"❌ خطا در {operation}:");
