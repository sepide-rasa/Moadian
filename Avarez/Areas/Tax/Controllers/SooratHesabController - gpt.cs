...
namespace Avarez.Areas.Tax.Controllers
{
    public class SooratHesabController : Controller
    {
        ...
        private static string LoadCertificateBase64(string certificatePath)
        {
            ...
            string text = System.IO.File.ReadAllText(certificatePath);
            return text;
        ...
        public async System.Threading.Tasks.Task<ActionResult> SamaneMoadian(long HeaderId)
            ...
            string path = "";
               ...
               // string invoiceJson = System.IO.File.ReadAllText(@"C:\invoices\sampleInvoice.json"); // یا build JSON
                ...
                // 1) nonce
                string nonce = await GetNonceAsync();
                Console.WriteLine("nonce: " + nonce);

                // 2) ساخت auth JWT با nonce
                string authJwt = CreateAuthJwtWithNonce(nonce, clientId, privateKeyPem, certificateCrt);
                Console.WriteLine("authJwt: " + authJwt);

                // 3) احراز و گرفتن access token
                string accessToken = await AuthenticateAsync(authJwt, clientId);
                Console.WriteLine("access token: " + accessToken);

                // 4) گرفتن server public key
                var serverInfo = await GetServerInformationAsync(accessToken);
                ...
                // 7) ارسال فاکتور
                string sendResponse = await SendInvoiceAsync(jwe, fiscalId, accessToken);
